    ...
    switch (operation) {
      ...
      case 'change':
        ...
        Object.keys(imports).forEach(function(name) {
          data[name] = {
            version: versionForModuleName(name, this.basedir)
          };
        }, this);
...
function versionForModuleName(name, basedir) {
  var resolve = require('resolve');
  var findUp = require('find-up');
  var path = require('path');
  var fs = require('fs');

  var cwd = path.dirname(resolve.sync(name, { basedir: basedir }));
  return JSON.parse(fs.readFileSync(findUp.sync('package.json', { cwd:  cwd }), 'UTF8')).version;
}
